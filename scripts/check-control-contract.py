...
UI 0.6.78 adds only the guarded Zone-7 interval 1→2 day target while retaining
the confirmed start-time probes, Program auto-refresh and generic one-shot writer.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.1"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.1"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.78"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0678.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
ui = (root / "custom_components" / "nikas_ho_sc_8w" / "frontend" / "irrigation-panel-v0678.mjs").read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.78"' in ui
assert 'import "./irrigation-panel-v0677.mjs"' in ui
assert 'const TARGET_FIELD = "cycle_value"' in ui
assert 'byte 15 01 → 02' in ui
