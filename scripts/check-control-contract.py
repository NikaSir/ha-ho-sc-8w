#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.6.93 / integration b006.14.

UI 0.6.93 keeps the guarded Zones editor / read-only Program split, restores
program_enabled editing through the proven DP38 flag patch path, normalizes
editor color semantics, stabilizes native pickers in the Zones drill-down, and
locks time-clear buttons to a separate fixed geometry column.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.14"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.14"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.93"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0693.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.14"' in manifest
assert 'PANEL_VERSION = "0.6.93"' in const
assert 'irrigation-panel-v0693.mjs' in const
...
ui_path = component / "frontend" / "irrigation-panel-v0693.mjs"
ui = ui_path.read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.93"' in ui
assert 'import "./irrigation-panel-v0692.mjs"' in ui
assert 'p._programEditorNativeControlActive = function programEditorNativeControlActiveV0693' in ui
assert 'this._view !== "program"' not in ui
assert 'grid-template-columns:minmax(0,1fr) 42px!important' in ui
assert 'grid-column:2!important' in ui
assert 'width:42px!important' in ui
assert 'overflow:hidden!important' in ui

for path in (editor_patch_path, lab_ui_path, readonly_ui_path, system_ui_path, editor_ui_path, ui_path):
