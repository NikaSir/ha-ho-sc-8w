#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.6.99 / integration b006.20."""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.20"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.20"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.99"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0699.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.20"' in manifest
assert 'PANEL_VERSION = "0.6.99"' in const
assert 'irrigation-panel-v0699.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 700)]
...
assert 'const UI_VERSION = "0.6.99"' in ui
assert 'Данные устарели' in ui
assert 'programSnapshotStale' in ui
assert 'warning-color' in ui
assert 'снимок устарел' in ui
