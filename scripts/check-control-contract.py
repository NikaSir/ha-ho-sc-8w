...
UI 0.6.83 adds a fixed Zone-7 program enabled->disabled byte-19 probe.
Integration b006.7 keeps the b006.3 read-only Program refresh and all idle-only
write guards.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.7"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.7"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.83"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0683.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
ui_path = root / "custom_components" / "nikas_ho_sc_8w" / "frontend" / "irrigation-panel-v0683.mjs"
ui = ui_path.read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.83"' in ui
assert 'import "./irrigation-panel-v0682.mjs"' in ui
assert 'const TARGET_FIELD = "program_enabled"' in ui
assert 'const TARGET_VALUE = "false"' in ui
assert 'const EXPECTED_SOURCE = "0711060C17FFFFFF1E2D3BFFFFFF00141A090410"' in ui
assert 'const EXPECTED_WRITE = "4011060C17FFFFFF1E2D3BFFFFFF00141A090400"' in ui
assert 'const EXPECTED_READ = "0711060C17FFFFFF1E2D3BFFFFFF00141A090400"' in ui
assert 'byte 19 10 → 00' in ui
assert 'data-zone7-program-off-prepare' in ui
assert 'data-zone7-program-off-execute' in ui
