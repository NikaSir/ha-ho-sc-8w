#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.6.96 / integration b006.17."""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.17"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.17"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.96"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0696.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.17"' in manifest
assert 'PANEL_VERSION = "0.6.96"' in const
assert 'irrigation-panel-v0696.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 697)]
for path in ui_paths:
    assert path.exists(), path
    subprocess.run(["node", "--check", str(path)], check=True)

ui = ui_paths[-1].read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.96"' in ui
assert 'Controller state is authoritative for every field the user has not touched.' in ui
assert 'state.intent instanceof Set' in ui
assert 'intent.has("program_enabled")' in ui
assert 'intent.has("repeat")' in ui
assert 'delete this._programDrafts[zone]' in ui
assert 'This never retries a write.' in ui
