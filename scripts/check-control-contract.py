#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.6.97 / integration b006.18."""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.18"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.18"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.97"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0697.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.18"' in manifest
assert 'PANEL_VERSION = "0.6.97"' in const
assert 'irrigation-panel-v0697.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 698)]
for path in ui_paths:
    assert path.exists(), path
    subprocess.run(["node", "--check", str(path)], check=True)

ui = ui_paths[-1].read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.97"' in ui
assert 'Для включённой программы длительность должна быть не меньше 1 минуты' in ui
assert 'programAnchorDateField' in ui
assert 'appearance:none!important' in ui
