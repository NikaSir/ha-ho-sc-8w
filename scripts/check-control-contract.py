#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.7.04 / integration b006.25."""
...
assert '"version": "1.0.0-b006.25"' in manifest
assert 'PANEL_VERSION = "0.7.04"' in const
assert 'irrigation-panel-v0704.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 705)]
...
assert 'const UI_VERSION = "0.7.04"' in ui
assert '_syncSingleProgramFreshnessV0704' in ui
assert 'legacyFresh?.remove()' in ui
assert '/Данные (свежие|устарели)/' in ui
