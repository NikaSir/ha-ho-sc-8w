...
UI 0.6.75 adds only the guarded Zone-7 start_time_1=06:30 research target.
The proven Program auto-refresh and generic one-shot DP38 writer stay intact.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b005.98"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b005.98"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.75"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0675.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert 'source[2:8] != b"\\xFF" * 6' in start_probe
assert 'source[8:14] != b"\\xFF" * 6' in start_probe
...
ui = (root / "custom_components" / "nikas_ho_sc_8w" / "frontend" / "irrigation-panel-v0675.mjs").read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.75"' in ui
assert 'import "./irrigation-panel-v0674.mjs"' in ui
assert 'field: "start_time_1", value: "06:30"' in ui
assert 'byte 2 FF → 06' in ui
assert 'byte 8 FF → 1E' in ui
