...
UI 0.6.81 adds a fixed Zone-7 Monday->Tuesday weekly-mask probe. Integration
b006.5 keeps the b006.3 read-only Program refresh and all idle-only write guards.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.5"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.5"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.81"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0681.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '0711060C17FFFFFF1E2D3BFFFFFF00021A090410' in start_probe
assert 'changes != [(0, "07", "40"), (15, "02", "04")]' in start_probe
...
ui_path = root / "custom_components" / "nikas_ho_sc_8w" / "frontend" / "irrigation-panel-v0681.mjs"
ui = ui_path.read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.81"' in ui
assert 'import "./irrigation-panel-v0680.mjs"' in ui
assert 'const TARGET_FIELD = "weekdays"' in ui
assert 'const TARGET_VALUE = "tue"' in ui
assert 'const EXPECTED_SOURCE = "0711060C17FFFFFF1E2D3BFFFFFF00021A090410"' in ui
assert 'const EXPECTED_WRITE = "4011060C17FFFFFF1E2D3BFFFFFF00041A090410"' in ui
assert 'const EXPECTED_READ = "0711060C17FFFFFF1E2D3BFFFFFF00041A090410"' in ui
assert 'byte 15 02 → 04' in ui
assert 'data-zone7-tue-prepare' in ui
assert 'data-zone7-tue-execute' in ui
