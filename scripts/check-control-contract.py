#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.7.02 / integration b006.23."""
...
assert '"version": "1.0.0-b006.23"' in manifest
assert 'PANEL_VERSION = "0.7.02"' in const
assert 'irrigation-panel-v0702.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 703)]
...
assert 'const UI_VERSION = "0.7.02"' in ui
assert '_syncActualPressureCardsV0702' in ui
assert 'nearestVisualCard' in ui
assert 'setProperty("background"' in ui
assert 'setProperty("border-color"' in ui
assert 'trim() === "Давление"' in ui
assert 'status?.title' in ui
