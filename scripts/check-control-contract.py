#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.7.03 / integration b006.24."""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.24"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.24"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.7.03"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0703.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.24"' in manifest
assert 'PANEL_VERSION = "0.7.03"' in const
assert 'irrigation-panel-v0703.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 704)]
...
assert 'const UI_VERSION = "0.7.03"' in ui
assert 'Данные актуальны · DP38' in ui
assert 'Данные устарели' in ui
assert 'Фактическая программа контроллера' not in ui
assert 'programReadFresh b::after{content:none' in ui
