#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.7.01 / integration b006.22."""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.22"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.22"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.7.01"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0701.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
assert '"version": "1.0.0-b006.22"' in manifest
assert 'PANEL_VERSION = "0.7.01"' in const
assert 'irrigation-panel-v0701.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 702)]
...
assert 'const UI_VERSION = "0.7.01"' in ui
assert '_syncPressureVisualToneV0701' in ui
assert 'pressureDanger' in ui and 'pressureWarning' in ui and 'pressureGood' in ui and 'pressureUnknown' in ui
assert 'hero.classList.remove(...HERO_TONES)' in ui
assert 'title === "давление"' in ui
