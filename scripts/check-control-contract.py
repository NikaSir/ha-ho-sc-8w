#!/usr/bin/env python3
"""Release safety contract for HO-SC-8W UI 0.7.00 / integration b006.21."""
...
assert '"version": "1.0.0-b006.21"' in manifest
assert 'PANEL_VERSION = "0.7.00"' in const
assert 'irrigation-panel-v0700.mjs' in const
...
ui_paths = [component / "frontend" / f"irrigation-panel-v{n:04d}.mjs" for n in range(688, 701)]
...
assert 'const UI_VERSION = "0.7.00"' in ui
assert 'value === 0' in ui and 'Нет давления' in ui
assert 'value < 0.3' in ui and 'Аварийно низкое' in ui
assert 'value < 2.5' in ui and 'Недостаточное' in ui
assert 'value <= 3.5' in ui and 'Норма' in ui
assert 'value <= 4.0' in ui and 'Повышенное' in ui
assert 'Аварийно высокое' in ui
assert 'Нет данных о давлении' in ui
