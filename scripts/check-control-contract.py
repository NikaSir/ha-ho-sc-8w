...
UI 0.6.82 adds a fixed Zone-7 Tuesday->Tuesday+Thursday weekly-mask probe.
Integration b006.6 keeps the b006.3 read-only Program refresh and all idle-only
write guards.
"""
...
source = source.replace('assert manifest["version"] == EXPECTED_INTEGRATION_VERSION','assert manifest["version"] == "1.0.0-b006.6"')
source = source.replace('assert panel_manifest["integration_version"] == manifest["version"]','assert panel_manifest["integration_version"] == EXPECTED_INTEGRATION_VERSION and manifest["version"] == "1.0.0-b006.6"')
source = source.replace('f\'PANEL_VERSION = "{EXPECTED_PANEL_VERSION}"\'','\'PANEL_VERSION = "0.6.82"\'')
source = source.replace('    EXPECTED_PANEL_BUNDLE,\n    "NUM_PRODUCTION_ZONES = 8",','    "irrigation-panel-v0682.mjs",\n    "NUM_PRODUCTION_ZONES = 8",')
...
ui_path = root / "custom_components" / "nikas_ho_sc_8w" / "frontend" / "irrigation-panel-v0682.mjs"
ui = ui_path.read_text(encoding="utf-8")
assert 'const UI_VERSION = "0.6.82"' in ui
assert 'import "./irrigation-panel-v0681.mjs"' in ui
assert 'const TARGET_FIELD = "weekdays"' in ui
assert 'const TARGET_VALUE = "tue,thu"' in ui
assert 'const EXPECTED_SOURCE = "0711060C17FFFFFF1E2D3BFFFFFF00041A090410"' in ui
...
assert 'byte 15 04 → 14' in ui
assert 'data-zone7-tue-thu-prepare' in ui
assert 'data-zone7-tue-thu-execute' in ui
