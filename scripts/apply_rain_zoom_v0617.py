...
def once(old, new):
    global p
    if old not in p: raise RuntimeError('missing marker: ' + old[:100])
...
marker='''        @media(max-width:430px){.appHeader{grid-template-columns:48px minmax(0,1fr) 48px;min-height:60px}.headerButton{width:44px;height:44px;border-radius:16px}.headerButton ha-icon{--mdc-icon-size:25px}.headerTitle strong{font-size:21px}.headerTitle small{font-size:13px}}'''
if marker not in p: raise RuntimeError('final media marker missing')
p=p.replace(marker, css+marker,1)
PANEL.write_text(p,encoding='utf-8')

const=Path('custom_components/nikas_ho_sc_8w/const.py'); s=const.read_text(); const.write_text(s.replace('PANEL_VERSION = "0.6.16"','PANEL_VERSION = "0.6.17"'))
