(() => {
  ...
  class HOSC8WPanel extends HTMLElement {
    ...
    headerTemplate(title, subtitle, compact = false) {
      return `
        <header class="appHeader">
          <button class="backButton" data-back aria-label="Назад"><ha-icon icon="mdi:arrow-left"></ha-icon><span>Назад</span></button>
          <div class="headerTitle ${compact ? "headerTitleCompact" : ""}"><strong>${this.esc(title)}</strong><small>${this.esc(subtitle)}</small></div>
    ...
    chip(label, display, entityId, tone = "") {
      return `<button class="chip ${tone}" data-entity="${this.esc(entityId)}"><small>${this.esc(label)}</small><strong>${this.esc(display)}</strong></button>`;
    }
    ...
    settingRow(label, value, entityId, icon, hint = "") {
      return `<button class="settingRow" data-entity="${this.esc(entityId)}"><span class="settingIcon"><ha-icon icon="${icon}"></ha-icon></span><span class="settingText"><b>${this.esc(label)}</b>${hint ? `<small>${this.esc(hint)}</small>` : ""}</span><strong>${this.esc(value)}</strong><ha-icon class="settingChevron" icon="mdi:chevron-right"></ha-icon></button>`;
    }
    ...
    bindActions() {
      ...
      this.shadowRoot.querySelectorAll("[data-entity]").forEach((button) => {
        ...
        button.addEventListener("click", (event) => { if (held) { event.preventDefault(); held = false; return; } if (zoneDetail) { this._drill = { type: "zone", zone: zoneDetail, parentView: "overview" }; this._view = "overview"; this.render(); window.scrollTo({ top: 0, behavior: "auto" }); return; } this.moreInfo(entityId); });
      });
    }
