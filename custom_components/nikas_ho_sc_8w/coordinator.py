...
class HOSC8WCoordinator(DataUpdateCoordinator[HOSC8WDevice]):
    ...
    async def async_test_zone8_anchor_date_write(
        self, confirmation: str
    ) -> dict[str, object]:
        """Run the one-write Zone 8 anchor-date probe and publish read-back."""
        async with self._transport_lock:
            try:
                result = await self.hass.async_add_executor_job(
                    self.api.test_zone8_anchor_date_write, confirmation
                )
            finally:
                self.async_set_updated_data(self.api.device)
            return result
