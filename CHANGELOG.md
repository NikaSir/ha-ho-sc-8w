...
- `1.0.0-b005.71` / panel `0.6.51`: emergency-disable and unregister the fixed Zone 8 anchor-date write after hardware read-back proved that the single DP38 block was not station-isolated: Zone 8 remained unchanged while Zone 4 was reset to zero minutes/no starts, changed from daily interval to weekly mask `127`, and received the test date `2026-09-02`. Diagnostics retains the decoded Zone 8 editor and raw all-zone observer as strictly read-only tools. The UI removes the write action, and the shared single-block writer now refuses every call before transport dispatch.
- `1.0.0-b005.70` / panel `0.6.50`: make the Zone 8 Diagnostics editor a factual decoder of the latest repeated valid raw DP38 response, including duration, all six start slots, cycle mode/value, anchor date and rain flag. Generic schedule writes and recovery remain disabled. Add one fixed, explicitly confirmed Zone 8 transport experiment that can change only the approved baseline anchor day `2026-09-03 → 2026-09-02` after local `OFF`/idle safety checks and two exact preflight reads. It sends one uppercase ASCII-HEX write only, then performs repeated read-back; mismatch never causes a retry or automatic rollback.
- `1.0.0-b005.69` / panel `0.6.49`: emergency-disable and unregister the guarded DP38 recovery after the controller acknowledged one write without changing Zone 8, indicating that DP38 station assignment may follow an internal rotating slot rather than the transmitted station byte. The read-only observer now continues until it has seen all Zones 1–8 and a repeated Zone 8 block (within the existing 24-request/12-second bound), labels every raw block against the exact saved backup for that station, and exposes whether the full round was completed. No DP38 write service is available in this release.
- `1.0.0-b005.68` / panel `0.6.48`: keep the raw DP38 collector running until Zone 8 itself has been returned identically at least twice (or the bounded 24-request/12-second limit is reached). Sequential one-zone controller replies no longer stop the recovery pre-check merely because six other unique zone blocks arrived first. No DP38 write is sent unless the exact approved damaged Zone 8 block is repeated.
- `1.0.0-b005.67` / panel `0.6.47`: add a single-purpose guarded recovery for Zone 8. It requires local transport, fresh DP101/107/108 confirming physical `OFF` and no watering, and at least two identical raw reads matching the exact observed damaged block `08EFEFEFEFEFEFFF8A9FDFEFFBFF007F1A090311`. Only then it writes the exact known backup `0800FFFFFFFFFFFFFFFFFFFFFFFF007F16010111` once and requires repeated exact read-back. Zones 1–6 remain blocked, generic DP38 writes remain disabled, and no automatic rollback is attempted after a mismatch. The diagnostic label now correctly identifies an invalid 20-byte station-8 reply as a damaged Zone 8 block.
- `1.0.0-b005.66` / panel `0.6.46`: replace the Zone-8-only filter with a strictly read-only raw DP38 observer. It keeps the existing persistent local socket, records every returned valid or invalid 20-byte block from any station, displays the observed DP set and request/response counts, and falls back to a clearly labelled cached Zone 8 block when no fresh DP38 arrives. No DP38 write is sent and all schedule write/restore paths remain disabled.
- `1.0.0-b005.65` / panel `0.6.45`: keep every valid read-only DP38 response for Zone 8 and expose each unique 40-character HEX block, count and decoded schedule fields. Alternating controller replies are now reported as observations instead of being discarded as a failed double-read; no DP38 write is sent and all schedule write/restore paths remain disabled.
- `1.0.0-b005.64` / panel `0.6.44`: replace the unattainable eight-zone DP38 comparison with a strictly read-only Zone 8 inspection. It requires two identical fresh valid Zone 8 blocks plus DP101/107/108 with physical `OFF` and no watering, then exposes the exact 40-character HEX block and refreshed decoded fields. It sends no DP38 write, does not toggle any bit and leaves all schedule write/restore paths disabled.
- `1.0.0-b005.63` / panel `0.6.43`: make the protected Zone 8 probe actively collect the controller's one-station-at-a-time DP38 reports using repeated status queries and TinyTuya's read-only `updatedps([38])` refresh request. No DP38 write is permitted until fresh reports for all eight zones and DP101/107/108 have been received; an incomplete round still stops before the first write and now reports the observed zones and request count.
