...
The `Программа` tab shows the complete decoded DP38 schedule for zones 1–6 and owns only the confirmed seasonal-correction editor. Production schedule editing is not exposed. The Zone 8 DP38 laboratory in Diagnostics is on a read-only incident hold after a Base64 payload was sent to the String DP38 `normal_time` field and produced invalid production schedules. Generic write and restore services remain disabled. A separate one-shot probe is available only for the physically unused Zone 8: with the controller physically `OFF`, no watering and local transport, it actively gathers the controller's one-station-at-a-time DP38 reports through repeated status and read-only `updatedps([38])` requests. Only after fresh DP101/107/108 and all eight DP38 blocks have arrived does it send one 20-byte station block as 40 uppercase ASCII HEX characters, verify a no-change write, toggle one Zone 8 rain-follow bit, compare Zones 1–7 byte-for-byte, and restore the exact original Zone 8 block. Production-zone recovery remains disabled until this probe passes on hardware.
